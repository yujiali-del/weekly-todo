(function () {
  ...
  window.demo = function (e, m) {
    ...
    toast(m || 'This links to a real page once connected to Shopify');
    return false;
  };

  // grip-sock canvas illustration, reused on homepage + shop grid
  window.drawSock = function (canvas, o) {
    ...
    pts.forEach(function (p) { x.beginPath(); x.arc(p[0], p[1], 1.8, 0, 7); x.fill(); });
    x.restore();
  };

  window.TENDUU_COLORS = [
    { name: 'Ballet Cream', base: '#fff3e6', cuff: '#F3C7A7', dot: '#E7C6B2' },
    { name: 'Sweet Butter', base: '#F4D98C', cuff: '#f0c96a', dot: '#8FA98C' },
    { name: 'Peach Blush', base: '#F3C7A7', cuff: '#eab08a', dot: '#5A4638' },
    { name: 'Nude Beige', base: '#E7C6B2', cuff: '#d5ac93', dot: '#5A4638' },
    { name: 'Sage Whisper', base: '#c3d2bd', cuff: '#a9bda1', dot: '#fffaf3' }
  ];
  ...
  // Guard against DOMContentLoaded having already fired before this listener
  // is registered (can happen depending on script load timing) — if the
  // document is already past 'loading', run immediately instead of waiting
